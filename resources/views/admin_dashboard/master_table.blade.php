         ...
         <div class="col-sm-12">
            <div class="page-title-box">
               <div class="btn-group pull-left">
                  <h4 class="viewpro"> <a href="{{ URL::previous() }}"><i class="fa fa-arrow-left" style="color: ##28a750; margin-right: 8px;"></i><span style="color: ##28a750; font-size: large; margin-right: 15px;">Back</span></a> Master Data Details</h4>
               </div>
               ...
               <div class="form-row">
                  <div class="form-group col-md-6">
                     ...
                     <select title="Select mode" placeholder="Select mode" class="form-control" id="master_key">
                        ...
                        <option value="trentygo_location">Trentygo Location</option>
                        @foreach($master_list as $ml)
                        <option value="{{ $ml->value }}" class="form-control">{{ ucfirst($ml->value) }}</option>
                        @endforeach
                     </select>
                  </div>
                  <div class="form-group col-md-6">
                     <label>Value</label><span style="color: red;">*</span>
                     <input data-parsley-type="number" type="text" class="form-control tribut" maxlength="30" required placeh older="Value..." id="value" />             
                  </div>
               </div>
               <div class="form-group">
                  <input type="hidden" id="csrf_token" value="{!! csrf_token() !!}">
                  <button type="button" class="btn btn-primary waves-effect waves-light" id="add_master_date">Add</button>
            ...
            <div class="card-box">
               ...
               <hr>
                  <div class="form-row">
                     <!--<div class="form-group col-md-6">-->
                     <!--   <label>Select Option</label>-->
                     <!--    <select title="Select mode" placeholder="Select mode" class="form-control" id="edit_master_key">-->
                     <!--       <option hidden="">Select Type</option>-->
                     <!--       @foreach($master_list as $ml)-->
                     <!--       <option class="form-control" value="{{ $ml->value }}" >{{ $ml->value }}</option>-->
                     <!--       @endforeach-->
                     <!--    </select>            -->
                     <!--</div>-->
                     <div class="form-group col-md-6">
                     ...
                     <div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
                        <div class="tab-pane  show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
                           ...
                           <form id="color_datatable">
                              <table class="table table-bordered dataTable no-footer mobile-table" id="master_list_datatable" style="table-layout:auto; width: 100%;">
                                 <thead>
                                    <tr>
                                       <th>#</th>
                                       <th>
...
<script>
   /*add color start*/
       $(function() {
   	   var table =  $('#master_list_datatable').DataTable({
               "pageLength":10,
               "processing":true,
               "serverSide": true,
   	        ajax: {
   	                url: '{{url('get_all_master_data')}}',
       ...
       $("#add_master_date").click(function(){
       ...
       function edit_master_tab(arg) {
        ...
        data: {
   	   ...
   	   function un_delete_color_info(arg,arg2) {
   	    var tempcsrf = $('#csrf_token').val();
   	    var master_data_id = arg;
   	    var status = arg2;
   	    $.confirm({
   	        title: 'Confirm!',
   	        content: 'Are you sure to delete this Master Date detail !!!',
   	        buttons: {
   	        confirm: function () {
   	          $.ajax({
   	            type: 'POST',
   	            url: '{{url('un_delete_color_list')}}',
   	            dataType: "json",
   	            data: {
   	                    master_data_id:master_data_id,
   	                    status:status,
